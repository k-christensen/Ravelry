...
u_list = user_json_and_profile('swiftmiss', 
pattern_pool = pattern_pool_df('swiftmiss', 'default_search'))
...
def pref_scores_exp(username, search = 'default_search', user_prof = 'None', save_user_profile = 'no'):
    pattern_pool = pattern_pool_df(username, search)
    user_list = user_json_and_profile(username, pattern_pool, user_prof, save_user_profile)
    u_json = user_list[0]
    u_p = user_list[1]
    ...
    # dictionary object: key = pattern id, value is percent match 
    predicted_user_prefs = {i:sum([a*b*c for a,b,c in zip(pool_idf, list(u_p.values()),pattern_pool.loc[i])])for i in pattern_pool.index}
